...
(async function(){
  ...
  function compactNumber(n){
    ...
    if(n >= 1000){
      ...
      const scaled = Math.round((n / 1000) * (10 ** digits)) / (10 ** digits);
  ...
  const citations = D.publications.reduce((sum,p)=>sum + (metricFor(p).citations || 0), 0);
  ...
  function animateCount(el){
    ...
    function fmt(v){
      if(isK){ return (Math.round((v/1000)*10)/10).toFixed(1).replace(/\.0$/,'')+'<span class="suf">k</span>'; }
  ...
  setTimeout(()=>{
    ...
    document.querySelectorAll('.stat .n').forEach(el=>{
      ...
      el.innerHTML = isK
        ? (Math.round((target/1000)*10)/10).toFixed(1).replace(/\.0$/,'')+'<span class="suf">k</span>'
